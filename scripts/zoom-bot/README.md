# Meeting bot (standalone)

Join a meeting and stream meeting audio to your voice server (e.g. port 3001). No Django, Celery, Redis, or Postgres.
...
| ZOOM_BOT_NAME | No | Display name in meeting (default "Gemini Sidekick") |
...
The Meeting SDK (Web) does **not** let you set a participant's avatar or profile image in code. The avatar is read-only and comes from the user's Zoom profile.

**Ways to get a logo for the bot:**

1. **Zoom user profile (recommended)**  
   If the bot joins using a **ZAK** or **on-behalf token** for a real Zoom user (e.g. a dedicated "Gemini Sidekick" account), that user's profile picture is used. Create a Zoom user for the bot, sign in at [zoom.us/profile](https://zoom.us/profile), and set the profile photo there. When the bot joins on behalf of that user, it will show that photo.

2. **App / SDK key only (no custom avatar)**  
   If the bot joins with only the Meeting SDK app credentials (no ZAK/OBF), Zoom shows the default placeholder (usually initials from the display name, e.g. "GS" for "Gemini Sidekick"). There is no API to set a custom image in this case.

So: for a custom logo, use a Zoom user account for the bot and set its profile picture in the Zoom web profile; otherwise the bot will show initials.
