...
- Create local environment variables
  - **Docker (Linux/WSL)**: `docker compose -f dev.docker-compose.yaml run --rm attendee-app-local python init_env.py > .env`
  - **Without Docker (Mac if Docker mount fails)**: `pip install cryptography 'Django>=5' && python init_env.py > .env`
  - **Windows (PowerShell with Docker)**: `docker compose -f dev.docker-compose.yaml run --rm attendee-app-local python init_env.py | Out-File -Encoding utf8 .env`
  - **Mac Docker "operation not permitted"**: Either use the "Without Docker" command above, or move the project to a folder Docker can access (e.g. `~/projects/attendee-main`) and run the Docker command from there. Ensure **Docker Desktop → Settings → Resources → File sharing** includes that path.
- Edit the `.env` file. Add your AWS credentials only if you need recording storage; you can leave `AWS_RECORDING_STORAGE_BUCKET_NAME`, `AWS_ACCESS_KEY_ID`, and `AWS_SECRET_ACCESS_KEY` empty if you do not need recording (see below).
...
If you don't need recording (only joining meetings and/or transcripts):

1. Leave AWS (or Azure) recording storage empty in `.env` — the app will use local filesystem storage and does not require S3/Azure.
2. When creating bots via the API, set `recording_settings` to `{"format": "none"}` so no recording file is produced. Transcripts and other features still work.
