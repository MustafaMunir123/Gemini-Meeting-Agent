...
# When no recording bucket/container is set, use local filesystem (allows running without AWS if you don't need recording)
RECORDING_STORAGE_DISABLED = not (AWS_RECORDING_STORAGE_BUCKET_NAME or AZURE_RECORDING_STORAGE_CONTAINER_NAME)
...
if RECORDING_STORAGE_DISABLED:
    # No S3/Azure configured: use local filesystem so the app runs without recording storage
    RECORDING_STORAGE_BACKEND = {
        "BACKEND": "django.core.files.storage.FileSystemStorage",
        "OPTIONS": {"location": str(BASE_DIR / "media" / "recordings")},
    }
    AUDIO_CHUNK_STORAGE_BACKEND = {
        "BACKEND": "django.core.files.storage.FileSystemStorage",
        "OPTIONS": {"location": str(BASE_DIR / "media" / "audio_chunks")},
    }
    DEFAULT_STORAGE_BACKEND = RECORDING_STORAGE_BACKEND
elif STORAGE_PROTOCOL == "azure":
